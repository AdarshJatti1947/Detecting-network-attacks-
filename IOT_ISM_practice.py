    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            while True:
                ...
                #response = f'Protocol Type: {protocol_type}\nService: {service}\nFlag: {flag}'.encode()
            ...
            # Create a list containing column names
                ...
                value=res()
                if(value==1):
                    response = f'Access Granted;  Connection status: {attacks_classification[value]}'.encode()
                    print("Access Granted")
                    conn.sendall(response)
                else:
                    response = f'Access Denied;   Intrusion type: {attacks_classification[value]}'.encode()
                    print("Access Denied")
                    conn.sendall(response)
